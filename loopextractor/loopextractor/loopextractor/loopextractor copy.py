...
def run_algorithm(audio_file, n_templates=[0,0,0], output_savename="extracted_loop"):
    ...
    for ith_loop in range(n_loops):
        ...
        bar_ind = choose_bar_to_reconstruct(factors[2], ith_loop, factors[1])
...
def get_downbeats(signal, fs, file):
    ...
    processor = madmom.features.downbeats.DBNDownBeatTrackingProcessor(beats_per_bar=4, fps=fs)
    downbeats_activation = madmom.features.downbeats.RNNDownBeatProcessor()(file)
    downbeats = processor(downbeats_activation)
    #print(downbeats)
    #print(downbeats_activation)
    #downbeats = processor(downbeats_activation)
    #downbeats = downbeats[:,1]
    """downbeats = []
    print(len(downbeats_activation))
    chunk_size = len(downbeats_activation)//25
    for start in range(0, downbeats_activation.shape[0], chunk_size):
        downbeats.append(processor(downbeats_activation[start : min(start + chunk_size,len(downbeats_activation))]))
        print(downbeats)"""
    return downbeats
...
def choose_bar_to_reconstruct(loop_templates, ith_loop, softmask_coefficients):
    ...
    bar_jnd = np.argmax(loop_templates[:,ith_loop]*np.sum(softmask_coefficients[:,ith_loop]))
    return bar_jnd
...
if __name__ == "__main__":
    ...
    run_algorithm("C:/Users/uno4e/OneDrive/Documents/Cambridge II/project/music/no_words/drum_and_bass/BoDleasons - Rhythm Reactor (Short 2 Version).mp3", [0,0,0], "C:/Users/uno4e/OneDrive/Documents/Cambridge II/project/test/sample")